...
class BaseActor<T> implements IActor<T> {
    user: T;
    constructor(user: T) {
        this.user = user;
    }
...
class Nobody extends BaseActor<null> {}

type User = {
    uid: string;
};

class SignedInUser extends BaseActor<User> {}
class SignedInUser2<T> extends BaseActor<T> {}

type Actor = Nobody | SignedInUser;

interface IAuthenticator {
    authorize<U, T extends IActor<U>>(actor: T): boolean;
}

class MyAuthenticator implements IAuthenticator {
    authorize<T extends SignedInUser>(actor: T): boolean {
        return true;
    }
}
